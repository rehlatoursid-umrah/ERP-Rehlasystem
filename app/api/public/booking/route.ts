...
export async function POST(request: Request) {
  try {
    ...
    if (!body.bookingCode?.trim()) {
      return NextResponse.json({ success: false, error: 'Kode booking wajib diisi' }, { status: 400 });
    }
    if (!body.phone?.trim()) {
      return NextResponse.json({ success: false, error: 'Nomor HP wajib diisi untuk verifikasi' }, { status: 400 });
    }
    ...
    if (!booking) {
      return NextResponse.json({ success: false, error: 'Kode booking tidak ditemukan' }, { status: 404 });
    }
    ...
    if (!phone || (!customerPhone.includes(phone) && !customerWa.includes(phone) && !phone.includes(customerPhone) && !phone.includes(customerWa))) {
      return NextResponse.json({ success: false, error: 'Nomor HP tidak cocok dengan data booking' }, { status: 403 });
    }
